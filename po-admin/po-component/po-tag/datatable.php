...
$currentRoleAccess = $tableroleaccess->findByAnd(id_level, $_SESSION['leveluser'], module, tag);
$currentRoleAccess = $currentRoleAccess->current();

if($currentRoleAccess->read_access == "Y"){

    $aColumns = array( 'id_tag', 'tag_title', 'tag_seo', 'count' );
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
        ...
        for ( $i=1 ; $i<count($aColumns) ; $i++ )
        {
			if($currentRoleAccess->delete_access == "Y"){
				$tbldelete = "<a class='btn btn-xs btn-danger alertdel' id='$aRow[id_tag]'><i class='fa fa-times'></i></a>";
			}
			$checkdata = "<div class='text-center'><input type='checkbox' id='titleCheckdel' /><input type='hidden' class='deldata' name='item[$no][deldata]' value='$aRow[id_tag]' disabled></div>";
			$row[] = $checkdata;
			$row[] = $aRow['id_tag'];
			$row[] = $aRow['tag_title'];
			$row[] = "<div class='text-center'>$aRow[count]</div>";
