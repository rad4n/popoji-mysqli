oTable = $('.dTableAjax').dataTable({
	"sAjaxSource": "po-component/po-gallery/datatable.php",
...
oTable2 = $('.dTableAjax2').dataTable({
	...
	"fnDrawCallback": function( oSettings ) {
		$(".alertdela").click(function(){
			var id = $(this).attr("id");
			$('#alertdela').modal('show');
			$('#delida').val(id);
		});
		$('.tblactive').click(function(){
			...
			$.ajax({
				type: "POST",
				url: "po-component/po-gallery/proses.php",
				data: dataString,
				cache: false,
