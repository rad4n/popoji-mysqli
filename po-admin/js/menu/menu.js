jQuery(function($) {
	...
	$('.edit-menu').live('click', function() {
		...
		gbox.show({
			type: 'ajax',
			url: 'po-component/po-menumanager/proses.php?mod=menu&act=edit&id=' + menu_id,
			buttons: {
				'Save': function() {
					$.ajax({
						type: 'POST',
	...
	$('.delete-menu').live('click', function() {
		...
		gbox.show({
			...
			buttons: {
				'Yes': function() {
					$.post('po-component/po-menumanager/proses.php?mod=menu&act=delete', param, function(data) {
						if (data.success) {
							gbox.hide();
							li.remove();
						} else {
	...
	$('#form-add-menu').submit(function() {
		if ($('#menu-title').val() == '') {
			...
			$.ajax({
				type: 'POST',
				url: 'po-component/po-menumanager/proses.php?mod=menu&act=add',
				data: $(this).serialize(),
				error: function() {
					gbox.show({
	...
	$('#add-group a').click(function() {
		gbox.show({
			type: 'ajax',
			url: 'po-component/po-menumanager/proses.php?mod=menu_group&act=add',
			buttons: {
				'Save': function() {
	...
	$('#edit-group').click(function() {
		...
		sgroup.html('<input type="text" class="text-group-input" value="' + group_title + '">');
		...
		inputgroup.focus().select().keydown(function(e) {
			if (e.which == 13) {
				...
				$.ajax({
					type: 'POST',
					url: 'po-component/po-menumanager/proses.php?mod=menu_group&act=edit',
	...
	$('#delete-group').click(function() {
		...
		gbox.show({
			...
			buttons: {
				'Yes': function() {
					$.post('po-component/po-menumanager/proses.php?mod=menu_group&act=delete', param, function(data) {
						if (data.success) {
